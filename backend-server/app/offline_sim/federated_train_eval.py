...
#Default CSV to the one generated by the new script
DEFAULT_OUT = os.path.join(BASE_DIR, "fl_model_v2.json")
DEFAULT_CSV = os.path.join(DATA_DIR, "synthetic_events.csv") 
...
def learn_group_weights_logit(train_df: pd.DataFrame) -> Dict[str, float]:
    ...
    X = pd.concat([
        ...
        one_hot_columns(train_df, "department"), 
    ], axis=1)
    
    y = train_df["is_anomaly"].astype(int).values 
    
    if X.shape[1] == 0:
        return {"location": 0.4, "device": 0.3, "time": 0.2, "department": 0.1}
    ...
    for col_group in ["location", "device", "time_bucket", "department"]:
    ...
    dept_w = group_abs.get("department", 0.0) 
    
    s = loc_w + dev_w + tim_w + dept_w 
    if s <= 1e-9:
        w = {"location": 0.4, "device": 0.3, "time": 0.2, "department": 0.1}
    else:
        w = {"location": loc_w/s, "device": dev_w/s, "time": tim_w/s, "department": dept_w/s}
...
def main():
    ...
    client_counts_dept = groupby_client_counts(train_df, key="department")
    ...
    model = save_model_v2(path=args.out, stats=stats, lp=lp, weights=weights,
                          threshold=0.5, aggregation_cfg=aggregation_cfg,
